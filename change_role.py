...
# Для reply menu клавятуры
async def handle_change_role_reply(message: Message, state: FSMContext):
    # Отправляет сообщение о начале процесса смены роли и переводит бота в состояние ожидания ID пользователя1
...
async def changed_role(callback: CallbackQuery, state: FSMContext):
    ...
    if clb_data in ["Судья", "Проверяющий", "Исполнитель", 'исключить', 'Админ'] and user[10]:
        ...
        if clb_data == "Судья":
            ...
            user_key = StorageKey(user_id=user[0], chat_id=user[0], bot_id=callback.bot.id, extra_param="error")
